...
from research.paper_replication.core.metrics import aggregate_metrics
from research.paper_replication.core.selection import select_pairs
from research.paper_replication.core.spread import build_spread, _ols_alpha_beta, SpreadSpec
from research.paper_replication.core.trading import simulate_pair
from research.paper_replication.core.wavelet import modwt_detail, DEFAULT_WAVELET
...
def collect_run(method, prices, periods, params, selections=None, tc_scenarios=None):
    ...
    default_tc = params.get("tc_scenario", research_config.PAIRS_CONFIG.get("tc_per_share", 0.001))
    if tc_scenarios is None or len(tc_scenarios) == 0:
        tc_list = [float(default_tc)]
    else:
        tc_list = [float(tc) for tc in tc_scenarios]
    headline_tc = next((tc for tc in tc_list if tc > 0.0), float(default_tc))

    std_parts, wav_parts, pf_parts, std_tc_parts, wav_tc_parts, idx_parts, bh_parts = [], [], [], [], [], [], []
    cat_rows, noise_rows = [], []
    selection_rows, trade_rows, transaction_rows, forced_rows = [], [], [], []
    ...
    for period in periods:
        ...
        for i, j in pairs:
            ...
            if s_std is not None:
                std_pair = simulate_pair(s_std)
                std_res.append(std_pair)
                for tc in tc_list:
                    std_tc_by[tc].append(simulate_pair(s_std, tc_per_share=tc))
                std_forced_res.append(simulate_pair(s_std, forced_close_only=True))
            ...
            if s_wav is not None:
                wav_pair = simulate_pair(s_wav)
                wav_res.append(wav_pair)
                for tc in tc_list:
                    wav_tc_by[tc].append(simulate_pair(s_wav, tc_per_share=tc))
            ...
            if s_pf is not None:
                pf_res.append(simulate_pair(s_pf))
        ...
        std_tc_parts.append(_portfolio_series(std_tc_by[headline_tc]))
        wav_tc_parts.append(_portfolio_series(wav_tc_by[headline_tc]))
        ...
        # Transaction-cost rows: tc=0.0 reference + each requested tc value.
        tc_emit = [
            ("standard", 0.0, std_res),
            ("wavelet", 0.0, wav_res),
        ]
        for tc in tc_list:
            tc_emit.append(("standard", tc, std_tc_by[tc]))
            tc_emit.append(("wavelet", tc, wav_tc_by[tc]))
        for variant, tc, res in tc_emit:
            report = aggregate_metrics(res, method, variant, n_pairs=len(pairs))
            transaction_rows.append({
                ...
                "pct_positive": report.pct_positive,
                "n_active": report.n_active,
                "n_trades": sum(len(pr.trades) for pr in res),
                "total_cost": sum(pr.total_cost for pr in res),
            })
        forced_report = aggregate_metrics(std_forced_res, method, "standard_forced_close", n_pairs=len(pairs))
        forced_rows.append({
            "period": period.index,
            "variant": "standard_forced_close",
            "mean_return": forced_report.mean_return,
            "sharpe": forced_report.sharpe,
            "pct_positive": forced_report.pct_positive,
            "n_active": forced_report.n_active,
            "n_trades": sum(len(pr.trades) for pr in std_forced_res),
        })
    ...
    data = RunData(
        ...
        transaction_stats=pd.DataFrame(transaction_rows),
        forced_close_stats=pd.DataFrame(forced_rows),
        example=example, pf_daily=_cat(pf_parts),
        std_tc_daily=_cat(std_tc_parts), wav_tc_daily=_cat(wav_tc_parts),
    )
    return data, cache
...
def _simulate_from_smooth(i, j, fi, fj, nt, dates, traw_i, traw_j, n_sigma):
    ...
    if not np.isfinite(sig) or sig == 0:
        return None
    from research.paper_replication.core.spread import SpreadSpec
...
def sweep_wavelets(method, prices, periods, params, selections, families):
    ...
    """
    from research.paper_replication.core.wavelet import modwt_smooth as _smooth
    n_sigma = params.get("threshold_sigma", 2.0)
    prepared = {}
    for period in periods:
        pairs, train_p, trade_p = selections[period.index]
...
def generate_all(prices, periods, params, methods=("distance", "cointegration"),
                 sweeps=True, save=True, tc_scenarios=None):
    ...
    """
    from research.paper_replication.analytics import asset_pricing as ap

    figures = {"fig01_pyramid": fig1_pyramid()}
    example_seen = False
    rf_usd = _riskfree_usd()
    factor_df, factor_source, factor_note = ap.load_available_factor_data()
    alpha_results = []
    run_payloads = {}
    wavelet_sweeps = {}
    horizon_sweeps = {}
    ...
    for method in methods:
        ...
        for variant, series in ap_variants:
            ...
            if res is not None:
                row = {"factor_family": "market", "method": method, "variant": variant, "model": "Market model",
                       ...
                       "R2": round(res.r2, 3), "adj_R2": round(res.adj_r2, 3), "n": res.n,
                       "factor_source": "local_equal_weight_market"}
                alpha_results.append(row)
            if factor_df is not None:
                alpha_results.extend(ap.run_paper_factor_models(
                    series, factor_df, method=method, variant=variant, source=factor_source,
                ))
        if factor_df is not None:
            tc = params.get("tc_per_share", research_config.PAIRS_CONFIG.get("tc_per_share", 0.0))
            for variant, series in (("standard_after_tc", data.std_tc_daily), ("wavelet_after_tc", data.wav_tc_daily)):
                if series is not None and not series.empty:
                    alpha_results.extend(ap.run_paper_factor_models(
                        series, factor_df, method=method, variant=variant,
                        source=factor_source, tc_per_share=tc,
                    ))
        elif factor_note:
            alpha_results.append({
                "factor_family": "factor_data_status", "method": method, "variant": "all",
                "model": "Fama-French/q-factor/Petkova", "note": factor_note,
            })
    ...
    if save:
        from research.paper_replication.outputs.output_writer import save_figure, save_table
