...
def modwt_level1_approx(
    ...
    This is the de-noised price series used in the paper for parameter
    estimation and spread construction.

    Algorithm
    ---------
    1. Symmetric padding: reflect tail to next power of 2
    2. FFT circular convolution with MODWT scaling filter g̃
    3. Trim to original length T

    Parameters
    ----------
    price_series : 1-D array, price levels (length T)
    wavelet      : filter name. 'sym22' = paper's primary filter.

    Returns
    -------
    approx : 1-D array of length T — the long-run component Ṽ_{1,t}
    """
    x = np.asarray(price_series, dtype=float)
    T = len(x)

    # Step 1: symmetric padding to dyadic length
    U = math.ceil(math.log2(T)) if T > 1 else 1
    T_ext = 2 ** U
    if T_ext < T:
        T_ext = 2 ** (U + 1)
    pad = T_ext - T
    x_ext = np.concatenate([x, x[-pad:][::-1]]) if pad > 0 else x.copy()

    # Step 2: FFT circular convolution
    g_tilde = _get_modwt_scaling_filter(wavelet)
    v_ext = np.real(np.fft.ifft(np.fft.fft(x_ext) * np.fft.fft(g_tilde, n=T_ext)))
